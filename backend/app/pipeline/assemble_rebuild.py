...
目视验收修复（2026-07-04，五项问题，历史记录）：
① 白底变黑 / ② 整页全黑 / ⑤ 矢量被盖：根因在 extract（SMask 未合并，黑基底
   实心化），本模块另加**白色垫底矩形**双保险（用户指定）；

已知限制（架构决策 A2，2026-07-04，取代上条①②⑤的根因修复）：extract 不再
合并 SMask（改为 assemble.py 原地保留 SMask 引用，见《压缩决策引擎.md》§5.4），
本模块直接吃 RasterImage.data_ref 字节、整页重画、没有 PDF 原生 SMask 可用，
真透明图会画成完全不透明（白色垫底矩形防不住，因为不透明图直接盖住它）。
本模块已退役、非当前路径，不为此单独恢复合并逻辑；测试见
test_assemble_rebuild.py::test_rebuild_black_scrim_regression（标记 xfail）。
未来若真正启用选项 C，需在绘制前自行合并 base+SMask。
